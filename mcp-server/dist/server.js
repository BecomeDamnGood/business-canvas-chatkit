...
const VERSION = process.env.VERSION ?? "v55";
...
function createAppServer() {
    ...
    server.registerTool("run_step", {
        title: "Run Step",
        description: "Widget-leading Business Strategy Canvas flow. Always call this tool for each user interaction and render using the widget output template. The user should answer via the widget (not the chat box).",
        inputSchema: {
        ...
        const user_message = current_step_id.toLowerCase() === "start" ? "" : user_message_raw;
        // NEW: capture seed ONLY for the initial start trigger (widget will use it once after Start)
        const seed_user_message = current_step_id.toLowerCase() === "start" && user_message_raw.trim()
            ? user_message_raw.trim()
            : "";
        // Basic request logging (so CloudWatch shows tool calls + failures)
        console.log(`[run_step] step_id=${current_step_id} user_message_len=${user_message_raw.length} state_keys=${Object.keys(state).length}`);
        try {
            ...
            const fallbackResult = {
                ...
                text: "",
                prompt: "Something went wrong on the server. Please try again (or restart the canvas).",
                specialist: {
                    action: "ESCAPE",
...
const httpServer = createServer(async (req, res) => {
    if (!req.url)
        return void res.writeHead(400).end("Missing URL");
