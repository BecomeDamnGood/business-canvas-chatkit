...
let runStepModule = null;
async function getRunStep() {
    if (!runStepModule)
        runStepModule = await import("./src/handlers/run_step.js");
...
function jsonRpcErrorResponse(status, message, data, code = -32700) {
    return {
        status,
        payload: {
            jsonrpc: "2.0",
...
async function runStepHandler(args) {
    ...
    const user_message = isStart && !user_message_raw.trim() ? "" : user_message_raw;
    const seed_user_message = isStart && user_message_raw.trim()
        ? user_message_raw.trim()
        : "";
    const hasInitiator = String(state?.initial_user_message ?? "").trim() !== "";
    const stateForTool = isStart && user_message_raw.trim()
        ? {
            ...state,
    ...
    try {
        ...
        const { debug: _omit, ...resultForClient } = result;
        const structuredContent = {
            title: `The Business Strategy Canvas Builder (${VERSION})`,
            meta: `step: ${result.state?.current_step ?? "unknown"} | specialist: ${result.active_specialist ?? "unknown"}`,
            result: resultForClient,
        };
        if (seed_user_message)
            structuredContent.seed_user_message = seed_user_message;
        ...
        const currentState = (stateForTool && typeof stateForTool === "object" && stateForTool !== null)
            ? stateForTool
            : getDefaultState();
        const currentStep = String(currentState.current_step || "step_0");
...
function createAppServer(baseUrl) {
    ...
    server.registerResource(UI_RESOURCE_NAME, uiResourceUri, {
    ...
    server.registerTool("run_step", {
        description: "Returns UI state for the business strategy canvas workflow. Accepts current step ID (optional, defaults to step_0), user message, and optional state object containing workflow progress.",
        ...
        const contentText = buildContentFromResult((structuredContent && structuredContent.result) ? structuredContent.result : null);
        return {
...
const httpServer = async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/favicon.ico") {
        const faviconPng = Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwEHgP5fFuuHAAAAAElFTkSuQmCC", "base64");
        res.writeHead(200, {
    ...
    if (req.method === "GET" && url.pathname === "/templates/presentation.pptx") {
        try {
            const filePath = path.join(process.cwd(), "assets", "presentation.pptx");
...
`;
    ...
    if (url.pathname === MCP_PATH && req.method && MCP_METHODS.has(req.method)) {
        ...
        try {
            ...
            const shouldPreParse = req.method === "POST" &&
