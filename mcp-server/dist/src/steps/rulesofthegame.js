...
import { z } from "zod";
export const RULESOFTHEGAME_STEP_ID = "rulesofthegame";
export const RULESOFTHEGAME_SPECIALIST = "RulesOfTheGame";
/**
 ...
 */
export function buildRulesOfTheGameSpecialistInput(userMessage, introShownForStep = "", currentStep = RULESOFTHEGAME_STEP_ID, language = "", previousStatements = []) {
 ...
 * Parse helper
 */
export function parseRulesOfTheGameOutput(raw) {
 ...
 */
export function normalizeRuleText(rule) {
    ...
    rules.forEach((rule, originalIndex) => {
        ...
        if (targetIndex === -1 && intents.length > 0) {
            for (let i = 0; i < mergedRules.length; i++) {
                ...
                if (!hasSharedIntent)
                    continue;
                const otherTokens = tokensByIndex[i] || new Set();
 ...
 */
export function classifyRuleIntent(rule) {
    const norm = normalizeRuleText(rule);
    if (!norm)
        return [];
    const intents = [];
    const text = ` ${norm} `;
    const hasAny = (keywords) => keywords.some((kw) => text.includes(` ${kw} `) || text.includes(` ${kw}-`) || text.includes(`${kw} `));
    if (hasAny([
        "innovation",
        "innovative",
        "innovating",
 ...
 */
export function enforceMaxRules(rules, max) {
    if (max <= 0) {
 ...
 */
export function postProcessRulesOfTheGame(statements, maxRules = 6) {
 ...
 */
export function buildRulesOfTheGameBullets(rules) {
    const lines = rules
 ...
 */
export function postProcessRulesOfTheGameFromBullets(raw, maxRules = 6) {
 ...
 */
export function buildUserFeedbackForRulesProcessing(result) {
    ...
    if (!hadMerges && !hadTruncation)
        return "";
    const parts = [];
    if (hadMerges) {
        const mergedCount = result.mergedGroups.reduce((sum, g) => sum + (Array.isArray(g.sourceIndices) ? g.sourceIndices.length : 0), 0);
        parts.push(mergedCount > 1
