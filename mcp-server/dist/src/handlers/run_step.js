...
import { CANONICAL_STEPS, getFinalsSnapshot, migrateState, CanvasStateZod, } from "../core/state.js";
...
import { TARGETGROUP_STEP_ID, TARGETGROUP_SPECIALIST, TARGETGROUP_INSTRUCTIONS, TargetGroupJsonSchema, TargetGroupZodSchema, buildTargetGroupSpecialistInput, } from "../steps/targetgroup.js";
import { PRODUCTSSERVICES_STEP_ID, PRODUCTSSERVICES_SPECIALIST, PRODUCTSSERVICES_INSTRUCTIONS, ProductsServicesJsonSchema, ProductsServicesZodSchema, buildProductsServicesSpecialistInput, } from "../steps/productsservices.js";
import { RULESOFTHEGAME_STEP_ID, RULESOFTHEGAME_SPECIALIST, RULESOFTHEGAME_INSTRUCTIONS, RulesOfTheGameJsonSchema, RulesOfTheGameZodSchema, buildRulesOfTheGameSpecialistInput, postProcessRulesOfTheGame, postProcessRulesOfTheGameFromBullets, buildRulesOfTheGameBullets, buildUserFeedbackForRulesProcessing, } from "../steps/rulesofthegame.js";
...
const STEP0_QUESTION_EN = "What type of venture is it, and what's the business name? If the name isn't final yet, write \"TBD\".";
function step0QuestionForLang(_lang) {
    return STEP0_QUESTION_EN;
}
function yesTokenForLang(_lang) {
    return "yes";
}
const PRESTART_WELCOME_DEFAULT = `Build a complete Business Model and Strategy Canvas step by step.
...
const UiStringsZodSchema = z.object(UI_STRINGS_KEYS.reduce((acc, k) => {
    acc[k] = z.string();
    return acc;
}, {}));
const UiStringsJsonSchema = {
    ...
    properties: UI_STRINGS_KEYS.reduce((acc, k) => {
        acc[k] = { type: "string" };
        return acc;
    }, {}),
};
const UI_STRINGS_CACHE = new Map();
...
function normalizePresentationTextSingle(input) {
    return String(input || "")
        .replace(/\r?\n+/g, " ")
        .replace(/\s+/g, " ")
        .trim();
}
function presentationLines(input) {
...
function sanitizeLinesForSection(lines, section) {
    const out = [];
    for (const line of lines) {
        ...
        if (detected) {
            if (detected.section !== section)
                break;
            if (detected.rest)
                out.push(detected.rest);
            continue;
        }
        out.push(cleaned);
    }
...
function hasNumberedLines(lines) {
    if (!lines || lines.length === 0)
        return false;
    const firstLine = lines[0].trim();
    return /^\d+[.)]\s/.test(firstLine);
}
function removeBulletsFromPPr(pPr) {
    if (!pPr)
        return pPr;
...
function buildParagraphXml(params) {
    ...
    parts.push("</a:r>");
    if (endParaRPr)
        parts.push(endParaRPr);
    parts.push("</a:p>");
    return parts.join("");
}
function replacePlaceholderParagraphs(xml, placeholder, lines) {
    const paraRe = /<a:p\b[\s\S]*?<\/a:p>/g;
    return xml.replace(paraRe, (paraXml) => {
        if (!paraXml.includes(`<a:t>${placeholder}</a:t>`))
            return paraXml;
...
function parseStep0Final(step0Final, fallbackName) {
...
function replacePlaceholdersInDir(rootDir, replacements, paragraphReplacements) {
...
function headingLabelsForLang(lang) {
...
function generatePresentationPptx(state, lang) {
    const templatePath = path.join(process.cwd(), "assets", "presentation.pptx");
    if (!fs.existsSync(templatePath)) {
        throw new Error("Presentation template not found");
    }
    const step0Final = String(state.step_0_final ?? "").trim();
    const fallbackName = String(state.business_name ?? "").trim();
    const { name } = parseStep0Final(step0Final, fallbackName);
    const labels = headingLabelsForLang(lang);
    const strategyLines = sanitizeLinesForSection(presentationLines(String(state.strategy_final ?? "")), "strategy");
    const targetGroupLines = sanitizeLinesForSection(presentationLines(String(state.targetgroup_final ?? "")), "targetgroup");
    const productsServicesLines = sanitizeLinesForSection(presentationLines(String(state.productsservices_final ?? "")), "productsservices");
    const rulesLines = sanitizeLinesForSection(presentationLines(String(state.rulesofthegame_final ?? "")), "rulesofthegame");
    const replacements = {
        "{{BUSINESS_NAME}}": escapeXml(normalizePresentationTextSingle(name || "TBD")),
        "{{BIG_WHY}}": escapeXml(normalizePresentationTextSingle(String(state.bigwhy_final ?? ""))),
        "{{BIGWHY}}": escapeXml(normalizePresentationTextSingle(String(state.bigwhy_final ?? ""))),
        "{{PURPOSE}}": escapeXml(normalizePresentationTextSingle(String(state.purpose_final ?? ""))),
        "{{ROLE}}": escapeXml(normalizePresentationTextSingle(String(state.role_final ?? ""))),
        "{{ENTITY}}": escapeXml(normalizePresentationTextSingle(String(state.entity_final ?? ""))),
        "{{DREAM}}": escapeXml(normalizePresentationTextSingle(String(state.dream_final ?? ""))),
...
function cleanupOldPresentationFiles(dir, maxAgeMs) {
    try {
        const now = Date.now();
 ...
 */
function processActionCode(actionCode, currentStep, state, lastSpecialistResult) {
    // Dream step ActionCodes
    if (actionCode === "ACTION_DREAM_INTRO_EXPLAIN_MORE")
        return "__ROUTE__DREAM_EXPLAIN_MORE__";
    if (actionCode === "ACTION_DREAM_INTRO_START_EXERCISE")
        return "__ROUTE__DREAM_START_EXERCISE__";
    if (actionCode === "ACTION_DREAM_WHY_GIVE_SUGGESTIONS")
        return "__ROUTE__DREAM_GIVE_SUGGESTIONS__";
    if (actionCode === "ACTION_DREAM_WHY_START_EXERCISE")
        return "__ROUTE__DREAM_START_EXERCISE__";
    if (actionCode === "ACTION_DREAM_SUGGESTIONS_PICK_ONE")
        return "__ROUTE__DREAM_PICK_ONE__";
    if (actionCode === "ACTION_DREAM_SUGGESTIONS_START_EXERCISE")
        return "__ROUTE__DREAM_START_EXERCISE__";
    if (actionCode === "ACTION_DREAM_REFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_DREAM_REFINE_START_EXERCISE")
        return "__ROUTE__DREAM_START_EXERCISE__";
    if (actionCode === "ACTION_DREAM_ESCAPE_CONTINUE")
        return "__ROUTE__DREAM_CONTINUE__";
    if (actionCode === "ACTION_DREAM_ESCAPE_FINISH_LATER")
        return "__ROUTE__DREAM_FINISH_LATER__";
    if (actionCode === "ACTION_DREAM_SWITCH_TO_SELF")
        return "__SWITCH_TO_SELF_DREAM__";
    // DreamExplainer step ActionCodes
    if (actionCode === "ACTION_DREAM_EXPLAINER_CONTINUE")
        return "__ROUTE__DREAM_EXPLAINER_CONTINUE__";
    if (actionCode === "ACTION_DREAM_EXPLAINER_FINISH_LATER")
        return "__ROUTE__DREAM_EXPLAINER_FINISH_LATER__";
    if (actionCode === "ACTION_DREAM_EXPLAINER_REFINE_CONFIRM")
        return "__ROUTE__DREAM_EXPLAINER_CONTINUE_TO_PURPOSE__";
    if (actionCode === "ACTION_DREAM_EXPLAINER_REFINE_ADJUST")
        return "__ROUTE__DREAM_EXPLAINER_REFINE__";
    if (actionCode === "ACTION_DREAM_EXPLAINER_NEXT_STEP")
        return "Next step";
    if (actionCode === "ACTION_DREAM_EXPLAINER_SUBMIT_SCORES")
        return "ACTION_DREAM_EXPLAINER_SUBMIT_SCORES";
    // Purpose step ActionCodes
    if (actionCode === "ACTION_PURPOSE_INTRO_EXPLAIN_MORE")
        return "__ROUTE__PURPOSE_EXPLAIN_MORE__";
    if (actionCode === "ACTION_PURPOSE_EXPLAIN_ASK_3_QUESTIONS")
        return "__ROUTE__PURPOSE_ASK_3_QUESTIONS__";
    if (actionCode === "ACTION_PURPOSE_EXPLAIN_GIVE_EXAMPLES")
        return "__ROUTE__PURPOSE_GIVE_EXAMPLES__";
    if (actionCode === "ACTION_PURPOSE_EXAMPLES_ASK_3_QUESTIONS")
        return "__ROUTE__PURPOSE_ASK_3_QUESTIONS__";
    if (actionCode === "ACTION_PURPOSE_EXAMPLES_CHOOSE_FOR_ME")
        return "__ROUTE__PURPOSE_CHOOSE_FOR_ME__";
    if (actionCode === "ACTION_PURPOSE_REFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_PURPOSE_REFINE_ADJUST")
        return "__ROUTE__PURPOSE_REFINE__";
    if (actionCode === "ACTION_PURPOSE_CONFIRM_SINGLE")
        return "yes";
    if (actionCode === "ACTION_PURPOSE_ESCAPE_CONTINUE")
        return "__ROUTE__PURPOSE_CONTINUE__";
    if (actionCode === "ACTION_PURPOSE_ESCAPE_FINISH_LATER")
        return "__ROUTE__PURPOSE_FINISH_LATER__";
    // Big Why step ActionCodes
    if (actionCode === "ACTION_BIGWHY_INTRO_GIVE_EXAMPLE")
        return "__ROUTE__BIGWHY_GIVE_EXAMPLE__";
    if (actionCode === "ACTION_BIGWHY_INTRO_EXPLAIN_IMPORTANCE")
        return "__ROUTE__BIGWHY_EXPLAIN_IMPORTANCE__";
    if (actionCode === "ACTION_BIGWHY_EXPLAIN_ASK_3_QUESTIONS")
        return "__ROUTE__BIGWHY_ASK_3_QUESTIONS__";
    if (actionCode === "ACTION_BIGWHY_EXPLAIN_GIVE_EXAMPLES")
        return "__ROUTE__BIGWHY_GIVE_EXAMPLES__";
    if (actionCode === "ACTION_BIGWHY_EXPLAIN_GIVE_EXAMPLE")
        return "__ROUTE__BIGWHY_GIVE_EXAMPLE__";
    if (actionCode === "ACTION_BIGWHY_REFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_BIGWHY_REFINE_ADJUST")
        return "__ROUTE__BIGWHY_REFINE__";
    if (actionCode === "ACTION_BIGWHY_ESCAPE_CONTINUE")
        return "__ROUTE__BIGWHY_CONTINUE__";
    if (actionCode === "ACTION_BIGWHY_ESCAPE_FINISH_LATER")
        return "__ROUTE__BIGWHY_FINISH_LATER__";
    // Role step ActionCodes
    if (actionCode === "ACTION_ROLE_INTRO_GIVE_EXAMPLES")
        return "__ROUTE__ROLE_GIVE_EXAMPLES__";
    if (actionCode === "ACTION_ROLE_INTRO_EXPLAIN_MORE")
        return "__ROUTE__ROLE_EXPLAIN_MORE__";
    if (actionCode === "ACTION_ROLE_ASK_GIVE_EXAMPLES")
        return "__ROUTE__ROLE_GIVE_EXAMPLES__";
    if (actionCode === "ACTION_ROLE_REFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_ROLE_REFINE_ADJUST")
        return "__ROUTE__ROLE_ADJUST__";
    if (actionCode === "ACTION_ROLE_ESCAPE_CONTINUE")
        return "__ROUTE__ROLE_CONTINUE__";
    if (actionCode === "ACTION_ROLE_ESCAPE_FINISH_LATER")
        return "__ROUTE__ROLE_FINISH_LATER__";
    if (actionCode === "ACTION_ROLE_EXAMPLES_CHOOSE_FOR_ME")
        return "__ROUTE__ROLE_CHOOSE_FOR_ME__";
    // Entity step ActionCodes
    if (actionCode === "ACTION_ENTITY_INTRO_FORMULATE")
        return "__ROUTE__ENTITY_FORMULATE__";
    if (actionCode === "ACTION_ENTITY_INTRO_EXPLAIN_MORE")
        return "__ROUTE__ENTITY_EXPLAIN_MORE__";
    if (actionCode === "ACTION_ENTITY_EXAMPLE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_ENTITY_EXAMPLE_REFINE")
        return "__ROUTE__ENTITY_REFINE__";
    if (actionCode === "ACTION_ENTITY_FORMULATE_FOR_ME")
        return "__ROUTE__ENTITY_FORMULATE_FOR_ME__";
    if (actionCode === "ACTION_ENTITY_ESCAPE_CONTINUE")
        return "__ROUTE__ENTITY_CONTINUE__";
    if (actionCode === "ACTION_ENTITY_ESCAPE_FINISH_LATER")
        return "__ROUTE__ENTITY_FINISH_LATER__";
    // Strategy step ActionCodes
    if (actionCode === "ACTION_STRATEGY_INTRO_FORMULATE")
        return "__ROUTE__STRATEGY_FORMULATE__";
    if (actionCode === "ACTION_STRATEGY_INTRO_EXPLAIN_MORE")
        return "__ROUTE__STRATEGY_EXPLAIN_MORE__";
    if (actionCode === "ACTION_STRATEGY_REFINE_EXPLAIN_MORE")
        return "__ROUTE__STRATEGY_EXPLAIN_MORE__";
    if (actionCode === "ACTION_STRATEGY_QUESTIONS_EXPLAIN_MORE")
        return "__ROUTE__STRATEGY_EXPLAIN_MORE__";
    if (actionCode === "ACTION_STRATEGY_ASK_3_QUESTIONS")
        return "__ROUTE__STRATEGY_ASK_3_QUESTIONS__";
    if (actionCode === "ACTION_STRATEGY_ASK_GIVE_EXAMPLES")
        return "__ROUTE__STRATEGY_GIVE_EXAMPLES__";
    if (actionCode === "ACTION_STRATEGY_ASK_WRITE")
        return "__ROUTE__STRATEGY_FORMULATE__";
    if (actionCode === "ACTION_STRATEGY_CONFIRM_SATISFIED")
        return "__ROUTE__STRATEGY_CONFIRM_SATISFIED__";
    if (actionCode === "ACTION_STRATEGY_FINAL_CONTINUE")
        return "__ROUTE__STRATEGY_FINAL_CONTINUE__";
    if (actionCode === "ACTION_STRATEGY_ESCAPE_CONTINUE")
        return "__ROUTE__STRATEGY_CONTINUE__";
    if (actionCode === "ACTION_STRATEGY_ESCAPE_FINISH_LATER")
        return "__ROUTE__STRATEGY_FINISH_LATER__";
    // Target Group step ActionCodes
    if (actionCode === "ACTION_TARGETGROUP_INTRO_EXPLAIN_MORE")
        return "__ROUTE__TARGETGROUP_EXPLAIN_MORE__";
    if (actionCode === "ACTION_TARGETGROUP_INTRO_ASK_QUESTIONS")
        return "__ROUTE__TARGETGROUP_ASK_QUESTIONS__";
    if (actionCode === "ACTION_TARGETGROUP_EXPLAIN_ASK_QUESTIONS")
        return "__ROUTE__TARGETGROUP_ASK_QUESTIONS__";
    if (actionCode === "ACTION_TARGETGROUP_POSTREFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_TARGETGROUP_POSTREFINE_ASK_QUESTIONS")
        return "__ROUTE__TARGETGROUP_ASK_QUESTIONS__";
    // Products and Services step ActionCodes
    if (actionCode === "ACTION_PRODUCTSSERVICES_CONFIRM")
        return "__ROUTE__PRODUCTSSERVICES_CONFIRM__";
    // Rules of the Game step ActionCodes
    if (actionCode === "ACTION_RULES_INTRO_WRITE")
        return "__ROUTE__RULES_WRITE__";
    if (actionCode === "ACTION_RULES_INTRO_EXPLAIN_MORE")
        return "__ROUTE__RULES_EXPLAIN_MORE__";
    if (actionCode === "ACTION_RULES_INTRO_GIVE_EXAMPLE")
        return "__ROUTE__RULES_GIVE_EXAMPLE__";
    if (actionCode === "ACTION_RULES_ASK_WRITE")
        return "__ROUTE__RULES_WRITE__";
    if (actionCode === "ACTION_RULES_ASK_GIVE_EXAMPLE")
        return "__ROUTE__RULES_GIVE_EXAMPLE__";
    if (actionCode === "ACTION_RULES_CONFIRM_ALL")
        return "__ROUTE__RULES_CONFIRM_ALL__";
    if (actionCode === "ACTION_RULES_REFINE_CONFIRM")
        return "yes";
    if (actionCode === "ACTION_RULES_REFINE_ADJUST")
        return "__ROUTE__RULES_ADJUST__";
    if (actionCode === "ACTION_RULES_ESCAPE_CONTINUE")
        return "__ROUTE__RULES_CONTINUE__";
    if (actionCode === "ACTION_RULES_ESCAPE_FINISH_LATER")
        return "__ROUTE__RULES_FINISH_LATER__";
    // Presentation step ActionCodes
    if (actionCode === "ACTION_PRESENTATION_CHANGE")
        return "__ROUTE__PRESENTATION_CHANGE__";
    if (actionCode === "ACTION_PRESENTATION_MAKE")
        return "__ROUTE__PRESENTATION_MAKE__";
    if (actionCode === "ACTION_PRESENTATION_ESCAPE_CONTINUE")
        return "__ROUTE__PRESENTATION_CONTINUE__";
    if (actionCode === "ACTION_PRESENTATION_ESCAPE_FINISH_LATER")
        return "__ROUTE__PRESENTATION_FINISH_LATER__";
    // Generic confirm/continue
    if (actionCode === "ACTION_CONFIRM_CONTINUE")
        return "yes";
 ...
 */
function mapChoiceTokenToRoute(choiceToken, currentStep, lastSpecialistResult) {
    const match = choiceToken.match(/^choice:([1-9])$/);
    if (!match)
        return choiceToken; // Geen choice token, doorlaten
    ...
    if (menuId) {
        ...
        if (menuId === "ROLE_MENU_INTRO") {
            if (choiceNum === "1")
                return "__ROUTE__ROLE_FORMULATE__";
            if (choiceNum === "2")
                return "__ROUTE__ROLE_GIVE_EXAMPLES__";
            if (choiceNum === "3")
                return "__ROUTE__ROLE_EXPLAIN_MORE__";
        ...
        if (menuId === "ENTITY_MENU_EXAMPLE") {
            if (choiceNum === "1")
                return "yes"; // Confirm and proceed to Strategy
            if (choiceNum === "2")
                return "__ROUTE__ENTITY_REFINE__"; // Refine - generate new formulation
        }
        ...
        if (menuId === "STRATEGY_MENU_QUESTIONS") {
            if (choiceNum === "1")
                return "__ROUTE__STRATEGY_EXPLAIN_MORE__"; // "Explain why I need a strategy"
        }
        if (menuId === "STRATEGY_MENU_CONFIRM") {
            if (choiceNum === "1")
                return "__ROUTE__STRATEGY_EXPLAIN_MORE__"; // "Explain why a Strategy matters"
            if (choiceNum === "2")
                return "__ROUTE__STRATEGY_CONFIRM_SATISFIED__"; // "I'm satisfied with my Strategy. Let's go to Rules of the Game"
        }
        ...
        if (menuId === "TARGETGROUP_MENU_INTRO") {
            if (choiceNum === "1")
                return "__ROUTE__TARGETGROUP_EXPLAIN_MORE__"; // "Explain me more about Target Groups"
            if (choiceNum === "2")
                return "__ROUTE__TARGETGROUP_ASK_QUESTIONS__"; // "Ask me some questions to define my specific Target Group"
        }
        ...
        if (menuId === "TARGETGROUP_MENU_POSTREFINE") {
            if (choiceNum === "1")
                return "yes"; // "Ja, dit is precies wat ik bedoel, ga naar stap Product and Services"
            if (choiceNum === "2")
                return "__ROUTE__TARGETGROUP_ASK_QUESTIONS__"; // "Ask me some questions to define my specific Target Group"
        }
        ...
        if (menuId === "PRODUCTSSERVICES_MENU_CONFIRM") {
            if (choiceNum === "1")
                return "__ROUTE__PRODUCTSSERVICES_CONFIRM__"; // "This is all what we offer, continue to step Rules of the Game"
        }
        // Rules menus
    ...
    if (currentStep === PURPOSE_STEP_ID) {
        if (prevAction === "REFINE") {
            ...
            if (choiceNum === "1")
                return "yes"; // Confirm
            if (choiceNum === "2")
                return "__ROUTE__PURPOSE_REFINE__"; // Refine further
        }
        if (prevAction === "ASK") {
    ...
    if (currentStep === BIGWHY_STEP_ID) {
        if (prevAction === "REFINE") {
            ...
            if (choiceNum === "1")
                return "yes"; // Confirm
            if (choiceNum === "2")
                return "__ROUTE__BIGWHY_REFINE__"; // Refine further
        }
        if (prevAction === "ASK") {
            // ASK: check menu context
            const prevQ = pickPrompt(lastSpecialistResult);
    ...
    if (currentStep === DREAM_STEP_ID) {
        if (prevAction === "REFINE") {
            // REFINE met 2-optie menu
            if (choiceNum === "1")
                return "yes"; // Confirm ("I'm happy with this wording...")
            if (choiceNum === "2")
                return "__ROUTE__DREAM_START_EXERCISE__"; // Start exercise
        }
        if (prevAction === "INTRO") {
            // INTRO: 2 options
            if (choiceNum === "1")
    ...
    if (currentStep === ROLE_STEP_ID) {
        if (prevAction === "REFINE") {
            // REFINE met 2-optie menu
            if (choiceNum === "1")
                return "yes"; // Confirm ("Yes, this fits.")
            if (choiceNum === "2")
                return "__ROUTE__ROLE_ADJUST__"; // Adjust
        }
        if (prevAction === "ASK") {
            // ASK: check menu context
            const prevQ = pickPrompt(lastSpecialistResult);
    ...
    if (currentStep === RULESOFTHEGAME_STEP_ID) {
        if (prevAction === "REFINE") {
            ...
            if (choiceNum === "1")
                return "yes"; // Confirm ("Yes, this fits")
            if (choiceNum === "2")
                return "__ROUTE__RULES_ADJUST__"; // Adjust
        }
        if (prevAction === "ASK") {
            // ASK: check menu context
            const prevQ = pickPrompt(lastSpecialistResult);
...
let cld3FactoryPromise = null;
let cld3Identifier = null;
async function getCld3Identifier() {
    if (cld3Identifier)
...
function countAlphaChars(input) {
    const s = String(input || "");
    const matches = s.match(/\p{L}/gu);
    return matches ? matches.length : 0;
}
async function detectLanguageHeuristic(text) {
    ...
    try {
        ...
        const reliable = typeof res.isReliable === "boolean" ? res.isReliable :
...
async function getUiStringsForLang(lang, model) {
...
async function ensureUiStringsForState(state, model) {
    const lang = normalizeLangCode(String(state.language ?? "")) || "en";
    const existingLang = String(state.ui_strings_lang ?? "").trim().toLowerCase();
    const existing = state.ui_strings;
...
async function ensureLanguageFromUserMessage(state, userMessage, model) {
    ...
    const override = String(state.language_override ?? "false") === "true";
...
function parseExplicitLanguageOverride(message) {
...
function buildSpecialistContextBlock(state) {
    ...
    const finalsLines = Object.keys(finals).length === 0
        ? "(none yet)"
...
export function applyStateUpdate(params) {
    ...
    if (next_step === TARGETGROUP_STEP_ID) {
        if (action === "CONFIRM" && typeof specialistResult?.targetgroup === "string") {
            ...
            if (firstSentence) {
                ...
                nextState.targetgroup_final = trimmed;
            }
        }
    }
    ...
    if (next_step === PRODUCTSSERVICES_STEP_ID) {
        if (action === "CONFIRM" && typeof specialistResult?.productsservices === "string") {
            ...
            if (v)
                nextState.productsservices_final = v;
        }
    }
    // ---- Rules of the Game ----
...
async function callSpecialistStrict(params) {
    ...
    if (specialist === STEP_0_SPECIALIST) {
        const langExplicit = String(state.language ?? "").trim();
    ...
    if (specialist === DREAM_SPECIALIST) {
        const langExplicitDream = String(state.language ?? "").trim();
        const plannerInput = buildDreamSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || DREAM_STEP_ID), langExplicitDream ? lang : "");
        const res = await callStrictJson({
            model,
    ...
    if (specialist === DREAM_EXPLAINER_SPECIALIST) {
        const langExplicitExplainer = String(state.language ?? "").trim();
        const fromLast = Array.isArray(state.last_specialist_result?.statements)
            ? state.last_specialist_result.statements
            : [];
        const fromScoring = Array.isArray(state.dream_scoring_statements)
            ? state.dream_scoring_statements
            : [];
        const previousStatements = fromScoring.length >= fromLast.length && fromScoring.length > 0 ? fromScoring : fromLast;
        const dreamAwaitingDirection = String(state.dream_awaiting_direction ?? "").trim() === "true";
        const topClusters = dreamAwaitingDirection && Array.isArray(state.dream_top_clusters)
            ? state.dream_top_clusters
            : undefined;
        const businessContext = dreamAwaitingDirection && topClusters
            ? {
                step_0_final: String(state.step_0_final ?? "").trim(),
                business_name: String(state.business_name ?? "").trim(),
            }
            : undefined;
        const plannerInput = buildDreamExplainerSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || DREAM_STEP_ID), langExplicitExplainer ? lang : "", previousStatements, topClusters, businessContext);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === PURPOSE_SPECIALIST) {
        const plannerInput = buildPurposeSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || PURPOSE_STEP_ID), lang);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === BIGWHY_SPECIALIST) {
        const plannerInput = buildBigWhySpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || BIGWHY_STEP_ID), lang);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === ROLE_SPECIALIST) {
        const plannerInput = buildRoleSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || ROLE_STEP_ID), lang);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === ENTITY_SPECIALIST) {
        const plannerInput = buildEntitySpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || ENTITY_STEP_ID), lang);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === STRATEGY_SPECIALIST) {
        const lastResult = state.last_specialist_result || {};
        const statementsFromLast = Array.isArray(lastResult.statements) ? lastResult.statements : [];
        const plannerInput = buildStrategySpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || STRATEGY_STEP_ID), lang, statementsFromLast);
        const res = await callStrictJson({
            model,
    ...
    if (specialist === TARGETGROUP_SPECIALIST) {
        const plannerInput = buildTargetGroupSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || TARGETGROUP_STEP_ID), lang, contextBlock);
        const res = await callStrictJson({
            model,
            instructions: `${TARGETGROUP_INSTRUCTIONS}\n\n${LANGUAGE_LOCK_INSTRUCTION}\n\n${contextBlock}\n\n${RECAP_INSTRUCTION}\n\n${UNIVERSAL_META_OFFTOPIC_POLICY}`,
            plannerInput,
            schemaName: "TargetGroup",
            jsonSchema: TargetGroupJsonSchema,
    ...
    if (specialist === PRODUCTSSERVICES_SPECIALIST) {
        const plannerInput = buildProductsServicesSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || PRODUCTSSERVICES_STEP_ID), lang, contextBlock);
        const res = await callStrictJson({
            model,
            instructions: `${PRODUCTSSERVICES_INSTRUCTIONS}\n\n${LANGUAGE_LOCK_INSTRUCTION}\n\n${contextBlock}\n\n${RECAP_INSTRUCTION}\n\n${UNIVERSAL_META_OFFTOPIC_POLICY}`,
            plannerInput,
            schemaName: "ProductsServices",
            jsonSchema: ProductsServicesJsonSchema,
    ...
    if (specialist === RULESOFTHEGAME_SPECIALIST) {
        const lastResult = state.last_specialist_result || {};
        const statementsFromLast = Array.isArray(lastResult.statements) ? lastResult.statements : [];
        const plannerInput = buildRulesOfTheGameSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || RULESOFTHEGAME_STEP_ID), lang, statementsFromLast);
        const res = await callStrictJson({
            model,
        ...
        if (data && typeof data === "object" && String(data.action) === "CONFIRM") {
            const statementsForProcessing = Array.isArray(data.statements)
                ? data.statements
                : [];
            ...
            if (feedback) {
                const baseMessage = typeof data.message === "string" ? String(data.message).trim() : "";
                data = {
                    ...data,
    ...
    if (specialist === PRESENTATION_SPECIALIST) {
        const plannerInput = buildPresentationSpecialistInput(userMessage, state.intro_shown_for_step, String(decision.current_step || PRESENTATION_STEP_ID), lang);
        const res = await callStrictJson({
            model,
...
export async function run_step(rawArgs) {
    ...
    const transientTextSubmit = typeof rawState.__text_submit === "string"
        ? String(rawState.__text_submit)
        : "";
    const transientPendingScores = Array.isArray(rawState.__pending_scores)
        ? rawState.__pending_scores
        : null;
    let state = migrateState(args.state ?? {});
    const fromArgs = String(rawState?.initial_user_message ?? "").trim();
    if (fromArgs && !String(state.initial_user_message ?? "").trim()) {
        state.initial_user_message = fromArgs;
    }
    ...
    const userMessageCandidate = pristineAtEntry ? rawNormalized : (looksLikeMetaInstruction(rawNormalized) ? "" : rawNormalized);
    ...
    if (String(state.initial_user_message ?? "").trim() === "" &&
        ...
        state.initial_user_message = String(userMessageCandidate).trim();
    }
    ...
    const lastSpecialistResult = state?.last_specialist_result;
    ...
    if (actionCodeRaw === "ACTION_TEXT_SUBMIT") {
        ...
        if (String(state.initial_user_message ?? "").trim() === "" &&
            submitted &&
            !/^[0-9]+$/.test(submitted)) {
            state.initial_user_message = submitted;
        }
    }
    ...
    const isUserTextForLang = msgForLang &&
        !/^[0-9]+$/.test(msgForLang) &&
        !msgForLang.startsWith("ACTION_") &&
        !msgForLang.startsWith("__ROUTE__") &&
        !msgForLang.startsWith("choice:");
    if (String(state.current_step) === STEP_0_ID &&
        String(state.step_0_final ?? "").trim() === "" &&
        isUserTextForLang) {
        const hasOverride = String(state.language_override ?? "false") === "true";
        if (!hasOverride) {
            state.language = "";
    ...
    if ((process.env.SKIP_MODE === "1" || true) && // Remove "|| true" voor productie veiligheid
        String(state.current_step) === STEP_0_ID &&
        String(userMessage ?? "").trim().startsWith("SKIP:")) {
        function parseSkipMode(input) {
            const result = {};
            // Remove SKIP: prefix
            ...
            if (step0Match) {
                ...
                const nameMatch = step0Text.match(/called\s+([^.,\n]+)/i) ||
                ...
                const status = hasRun ? "existing" : hasStart ? "starting" : "existing";
        ...
        if (parsed.step0) {
            ...
            state.step_0_final = step0Final;
            state.business_name = parsed.step0.name;
        }
        if (parsed.dream) {
            state.dream_final = parsed.dream;
        }
        if (parsed.purpose) {
            state.purpose_final = parsed.purpose;
        }
        if (parsed.bigwhy) {
            state.bigwhy_final = parsed.bigwhy;
        }
        if (parsed.role) {
            state.role_final = parsed.role;
        }
        if (parsed.entity) {
            state.entity_final = parsed.entity;
        }
        if (parsed.strategy) {
            state.strategy_final = parsed.strategy;
        }
        if (parsed.targetgroup) {
            state.targetgroup_final = parsed.targetgroup;
        }
        if (parsed.productsservices) {
            state.productsservices_final = parsed.productsservices;
        }
        if (parsed.rulesofthegame) {
            const processed = postProcessRulesOfTheGameFromBullets(parsed.rulesofthegame, 6);
            state.rulesofthegame_final = processed.bulletList;
        }
        if (parsed.presentation) {
            state.presentation_brief_final = parsed.presentation;
        }
        ...
        let nextStep = String(PURPOSE_STEP_ID);
        if (parsed.dream) {
        ...
        state.intro_shown_for_step = ""; // Reset so intro will be shown for new step
        state.started = "true";
    ...
    function requireFinalValue(stepId, prev, stateObj) {
        if (stepId === STEP_0_ID) {
            return { field: "step_0_final", value: pickFirstNonEmpty(prev.step_0, stateObj.step_0_final) };
        }
        if (stepId === DREAM_STEP_ID) {
            return { field: "dream_final", value: pickFirstNonEmpty(prev.dream, prev.refined_formulation, stateObj.dream_final) };
        }
        if (stepId === PURPOSE_STEP_ID) {
            return { field: "purpose_final", value: pickFirstNonEmpty(prev.purpose, prev.refined_formulation, stateObj.purpose_final) };
        }
        if (stepId === BIGWHY_STEP_ID) {
            return { field: "bigwhy_final", value: pickFirstNonEmpty(prev.bigwhy, prev.refined_formulation, stateObj.bigwhy_final) };
        }
        if (stepId === ROLE_STEP_ID) {
            return { field: "role_final", value: pickFirstNonEmpty(prev.role, prev.refined_formulation, stateObj.role_final) };
        }
        if (stepId === ENTITY_STEP_ID) {
            return { field: "entity_final", value: pickFirstNonEmpty(prev.entity, prev.refined_formulation, stateObj.entity_final) };
        }
        if (stepId === STRATEGY_STEP_ID) {
            return { field: "strategy_final", value: pickFirstNonEmpty(prev.strategy, prev.refined_formulation, stateObj.strategy_final) };
        }
        if (stepId === TARGETGROUP_STEP_ID) {
            return { field: "targetgroup_final", value: pickFirstNonEmpty(prev.targetgroup, prev.refined_formulation, stateObj.targetgroup_final) };
        }
        if (stepId === PRODUCTSSERVICES_STEP_ID) {
            return { field: "productsservices_final", value: pickFirstNonEmpty(prev.productsservices, prev.refined_formulation, stateObj.productsservices_final) };
        }
        if (stepId === RULESOFTHEGAME_STEP_ID) {
            return { field: "rulesofthegame_final", value: pickFirstNonEmpty(prev.rulesofthegame, prev.refined_formulation, stateObj.rulesofthegame_final) };
        }
        if (stepId === PRESENTATION_STEP_ID) {
            return { field: "presentation_brief_final", value: pickFirstNonEmpty(prev.presentation_brief, prev.refined_formulation, stateObj.presentation_brief_final) };
        }
        return { field: "", value: "" };
    }
    function normalizeConfirmFinals(stepId, result, currentState, langCode) {
        if (!result || String(result.action || "") !== "CONFIRM")
            return result;
        const confirmationPrompt = String(result.confirmation_question ?? "").trim() || String(result.question ?? "").trim();
        ...
        if (stepId === STEP_0_ID) {
            ...
            if (step0)
                return result;
            const businessName = String(result.business_name ?? currentState.business_name ?? "TBD").trim() || "TBD";
            return {
                ...result,
                action: "ASK",
                message: "",
        ...
        if (refined) {
            if (stepId === DREAM_STEP_ID)
                return { ...result, dream: refined };
            if (stepId === PURPOSE_STEP_ID)
                return { ...result, purpose: refined };
            if (stepId === BIGWHY_STEP_ID)
                return { ...result, bigwhy: refined };
            if (stepId === ROLE_STEP_ID)
                return { ...result, role: refined };
            if (stepId === ENTITY_STEP_ID)
                return { ...result, entity: refined };
            if (stepId === STRATEGY_STEP_ID)
                return { ...result, strategy: refined };
            if (stepId === TARGETGROUP_STEP_ID) {
            ...
            if (stepId === PRODUCTSSERVICES_STEP_ID)
                return { ...result, productsservices: refined };
            if (stepId === RULESOFTHEGAME_STEP_ID)
                return { ...result, rulesofthegame: refined };
            if (stepId === PRESENTATION_STEP_ID)
                return { ...result, presentation_brief: refined };
        }
        return {
            ...result,
            action: "ASK",
    ...
    if (actionCodeRaw && HARD_CONFIRM_ACTIONS.has(actionCodeRaw)) {
        ...
        if (finalInfo.field && !finalInfo.value) {
            ...
            if (finalInfo.field && finalInfo.value) {
                state[finalInfo.field] = finalInfo.value;
            }
            state.last_specialist_result = nextLast;
            userMessage = "";
            forcedProceed = true;
        }
    }
    ...
    const initialUserMessage = String(state.initial_user_message ?? "").trim();
    if (userMessage.trim() === "" &&
        initialUserMessage &&
    ...
    if (state.current_step === PRESENTATION_STEP_ID && userMessage === "__ROUTE__PRESENTATION_MAKE__") {
        try {
            console.log("[presentation] Generate requested", {
                cwd: process.cwd(),
                hasTemplate: fs.existsSync(path.join(process.cwd(), "assets", "presentation.pptx")),
            });
            ...
            const message = lang.startsWith("nl")
    ...
    const isDreamStepExplainer = state.current_step === DREAM_STEP_ID &&
        String(state.active_specialist ?? "") === DREAM_EXPLAINER_SPECIALIST;
    ...
    if (isDreamStepExplainer && userMessage.trim().length > 0) {
        if (userMessage === "ACTION_DREAM_EXPLAINER_SUBMIT_SCORES") {
            if (Array.isArray(transientPendingScores))
                parsedScores = transientPendingScores;
        }
        else {
            try {
    ...
    if (isDreamStepExplainer && parsedScores && parsedScores.length > 0) {
        ...
        const statementsFromLast = Array.isArray(lastResult.statements) ? lastResult.statements : [];
        const statements = statementsFromLast.length > 0
            ? statementsFromLast
            : Array.isArray(state.dream_scoring_statements)
                ? state.dream_scoring_statements
                : [];
        if (clusters.length === parsedScores.length && statements.length > 0) {
            const clusterAverages = clusters.map((c, ci) => {
                ...
                return { theme: String(c.theme ?? "").trim() || `Category ${ci + 1}`, average };
            });
            ...
            nextStateScores.dream_scores = parsedScores;
            nextStateScores.dream_top_clusters = topClusters;
            nextStateScores.dream_awaiting_direction = "true";
            ...
            const forcedDecision = {
                ...
                intro_shown_for_step: String(state.intro_shown_for_step ?? "").trim() || "dream",
                intro_shown_session: String(state.intro_shown_session ?? "").trim() === "true" ? "true" : "false",
            ...
            nextStateFormulation.dream_awaiting_direction = "false";
    ...
    if (userMessage.trim() === SWITCH_TO_SELF_DREAM_TOKEN && state.current_step === DREAM_STEP_ID) {
        state.intro_shown_for_step = "dream";
        const forcedDecision = {
    ...
    if (isStartTrigger) {
        ...
        if (!started) {
            ...
            const startHint = typeof stateWithUi.ui_strings?.startHint === "string"
                ? String(stateWithUi.ui_strings.startHint)
        ...
        const existingFirst = state.last_specialist_result;
        const isReuseFirst = existingFirst &&
        ...
        if (step0Final) {
            ...
            const confirmReady = " Is that correct, and if so are you ready to start the first step, 'Your Dream'?";
            const specialist = {
                action: "CONFIRM",
                message: "",
                question: "",
        ...
        const initialMsg = String(state.initial_user_message ?? "").trim();
        if (!String(state.language ?? "").trim() && initialMsg) {
    ...
    const prevConfirm = state.last_specialist_result || {};
    const confirmAction = String(prevConfirm?.action ?? "") === "CONFIRM";
    const confirmPromptNonEmpty = typeof prevConfirm?.confirmation_question === "string" &&
        String(prevConfirm.confirmation_question).trim() !== "";
    const stepId = String(state.current_step ?? "").trim();
    const hasFinal = (stepId === DREAM_STEP_ID && String(state.dream_final ?? "").trim() !== "") ||
        (stepId === PURPOSE_STEP_ID && String(state.purpose_final ?? "").trim() !== "") ||
        (stepId === BIGWHY_STEP_ID && String(state.bigwhy_final ?? "").trim() !== "") ||
        (stepId === ROLE_STEP_ID && String(state.role_final ?? "").trim() !== "") ||
        (stepId === ENTITY_STEP_ID && String(state.entity_final ?? "").trim() !== "") ||
        (stepId === STRATEGY_STEP_ID && String(state.strategy_final ?? "").trim() !== "") ||
        (stepId === TARGETGROUP_STEP_ID && String(state.targetgroup_final ?? "").trim() !== "") ||
        (stepId === PRODUCTSSERVICES_STEP_ID && String(state.productsservices_final ?? "").trim() !== "") ||
        (stepId === RULESOFTHEGAME_STEP_ID &&
            String(state.rulesofthegame_final ?? "").trim() !== "") ||
        (stepId === PRESENTATION_STEP_ID &&
            String(state.presentation_brief_final ?? "").trim() !== "");
    ...
    if (confirmDetected && userAffirmed) {
        const idx = CANONICAL_STEPS.indexOf(stepId);
    ...
    const lastResult = state.last_specialist_result || {};
    const dreamReadinessYes = state.current_step === DREAM_STEP_ID &&
        String(lastResult.suggest_dreambuilder ?? "") === "true" &&
        isClearYes(userMessage);
    const dreamReadinessFallback = state.current_step === DREAM_STEP_ID &&
        isClearYes(userMessage) &&
    ...
    if (useDreamExplainerGuard) {
        const forcedDecision = {
            ...
            current_step: DREAM_STEP_ID,
            intro_shown_for_step: String(state.intro_shown_for_step ?? ""),
            intro_shown_session: state.intro_shown_session === "true" ? "true" : "false",
            show_step_intro: "false",
            show_session_intro: "false",
        };
        ...
        const normalizedDreamExplainer = normalizeConfirmFinals(String(forcedDecision.current_step || ""), callDreamExplainer.specialistResult, state, lang);
        ...
        return {
            ...
            active_specialist: String(nextStateDream.active_specialist || ""),
            text: textDream,
            prompt: promptDream,
    ...
    if (decision1.specialist_to_call === DREAM_EXPLAINER_SPECIALIST &&
        ...
        const prevStatements = Array.isArray(state.last_specialist_result?.statements)
            ? state.last_specialist_result.statements
            : [];
    ...
    specialistResult = normalizeConfirmFinals(String(decision1.current_step || ""), specialistResult, state, lang);
        ...
        if (stmtCount >= 20) {
            ...
            if (decisionNext.specialist_to_call === DREAM_EXPLAINER_SPECIALIST) {
                ...
                if (call2.specialistResult && String(call2.specialistResult.scoring_phase ?? "") === "true") {
                    ...
                    specialistResult = normalizeConfirmFinals(String(decisionNext.current_step || ""), call2.specialistResult, stateAfterFirst, lang);
                }
            }
        }
    }
    ...
    if (decision1.specialist_to_call === DREAM_EXPLAINER_SPECIALIST &&
        String(state.dream_awaiting_direction ?? "").trim() === "true") {
        nextState.dream_awaiting_direction = "false";
    }
    if (decision1.specialist_to_call === DREAM_EXPLAINER_SPECIALIST &&
        specialistResult &&
        Array.isArray(specialistResult.statements) &&
        specialistResult.statements.length >= 20) {
        nextState.dream_scoring_statements = specialistResult.statements;
    }
    ...
    if (shouldChainToNextStep(decision1, specialistResult)) {
        ...
        if (String(decision2.specialist_to_call || "") && String(decision2.current_step || "")) {
            ...
            attempts = Math.max(attempts, call2.attempts);
            specialistResult = normalizeConfirmFinals(String(decision2.current_step || ""), call2.specialistResult, nextState, lang);
