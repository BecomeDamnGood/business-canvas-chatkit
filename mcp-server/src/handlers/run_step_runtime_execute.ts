...
export async function runStepRuntimeExecute(
  ...
  const actionRoutingLayer = await runStepRuntimeActionRoutingLayer({
    ...
    action: {
      pretransitionByActionCode: ACTION_PRETRANSITION_BY_ACTIONCODE as unknown as Record<string, unknown>,
  ...
  const pipelinePorts = {
    ids: { step0Id: STEP_0_ID, dreamStepId: DREAM_STEP_ID, bigwhyStepId: BIGWHY_STEP_ID, strategyStepId: STRATEGY_STEP_ID, dreamSpecialist: DREAM_SPECIALIST, dreamExplainerSpecialist: DREAM_EXPLAINER_SPECIALIST, strategySpecialist: STRATEGY_SPECIALIST, dreamExplainerSwitchSelfMenuId: "" },
    policy: {
