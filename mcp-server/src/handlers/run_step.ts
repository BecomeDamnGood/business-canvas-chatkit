...
const STEP0_QUESTION_NL =
  "Wat voor soort onderneming start of run je, en wat is de naam van je bedrijf (of is die nog TBD)?";

function step0QuestionForLang(lang: string): string {
  const l = String(lang || "").toLowerCase();
  return l.startsWith("nl") ? STEP0_QUESTION_NL : STEP0_QUESTION_EN;
}

function yesTokenForLang(lang: string): string {
  const l = String(lang || "").toLowerCase();
  if (l.startsWith("nl")) return "ja";
  if (l.startsWith("fr")) return "oui";
  if (l.startsWith("es")) return "sí";
  if (l.startsWith("it")) return "sì";
  if (l.startsWith("pt")) return "sim";
  if (l.startsWith("de")) return "ja";
  return "yes";
}
...
function detectLanguageHeuristic(text: string): string {
  ...
  const pad = ` ${s} `;
  const count = (arr: string[]) => arr.reduce((acc, w) => acc + (pad.includes(w) ? 1 : 0), 0);
...
function ensureLanguageFromUserMessage(state: CanvasState, userMessage: string): CanvasState {
  ...
  if (locked && current) return state;

  // Do not lock language on pure numeric menu clicks.
  if (/^[0-9]+$/.test(msg)) return state;
