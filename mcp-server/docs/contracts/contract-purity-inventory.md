...
| Widget render-state SSOT (`_meta.widget_result` + ordering tuple) | UI render authority contract | `mcp-server/ui/step-card.bundled.html`, `mcp-server/src/server/http_routes.ts`, `mcp-server/src/server/run_step_transport.ts` | Impliciet (ADR-006 policy) | Contract-tests + `guard:ssot:ui` op bundled runtime authority wrappers en server ownership; fail-closed bij missende/corrupte authority source | `Partieel BC` | Legacy `root.result` en `_widget_result` zijn expliciet non-authority. |
