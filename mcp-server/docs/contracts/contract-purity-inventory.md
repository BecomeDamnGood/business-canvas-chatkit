...
| Legacy action code/token patroon | Legacy grammar contract | `mcp-server/src/contracts/legacy_actioncodes.ts` | Geen expliciete versie | Regex-validatie (`LegacyActionCodeZod`, `LegacyRouteTokenZod`) | `Partieel BC` | Beperkt tot patroonvalidering, niet semantische geldigheid. |
...
| MCP `run_step` input (`RunStepInputSchema`) | MCP tool API input | `mcp-server/server.ts` | Impliciet (release) | `z.object`; state via `z.preprocess(canonicalizeStateForToolInput, CanvasStateZod.partial().passthrough())` | `Partieel BC` | Bewust lenient voor state (partial+passthrough) en defaults op inputvelden. |
