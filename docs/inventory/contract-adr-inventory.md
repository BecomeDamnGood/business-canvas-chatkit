...
| Tool API input/output (`run_step`) | `server.ts` | [ADR-002](../adr/ADR-002-contract-versioning-policy.md), [ADR-003](../adr/ADR-003-run-step-idempotency-replay.md) | Schema versie + deprecatiepad | Schema diff + replay invariants |
...
| Idempotency/replay en transities | `handlers/ingress.ts`, `handlers/run_step_runtime.ts`, `contracts/transitions.ts` | [ADR-003](../adr/ADR-003-run-step-idempotency-replay.md) | Turn-key semantiek expliciet | Replay regression tests |
